...
LEADS_Input :: LEADS_Input()
{
  ...
  //RunParameters
   ...
   TimeStep          = 1.0E-2; // Simulation time step in 'femtoseconds' or 'dimensionless'
  ...
  // Laser Parameters
   ...
   a0         = var_value;                                              // Laser amplitude
   Tau_FWHM   = 4.0 * (2*pi);                                        // Duration in cycles
   //a0       = sqrt(Intensity*SQ(Wavelength/1e-6)/1.36627476e18); // Laser amplitude
   //Tau_FWHM = 10.0 * (Omega * fs);                               // Duration in fs

   PulseProfile = 1;    // 0-Plane infinite wave; 1-Gaussian Laser pulse
  ...
  // Particle Beam Parameters
   ...
   BunchLength = pi;       // Length of cyclindrical bunch (will count from z0/x0)
   BunchRadius = 2*pi*2;       // Radius of cylindrical bunch (will be centred around x0,y0)
   ...
   double RayleighRange = (pi * SQ(BeamWaist * um)/Wavelength) * (2*pi/Wavelength); 
  ...
  //Diagnostics: Spectrum Module
   ...
   WritePhaseSpace = true;    // true/false: store phase space
   PhaseStoreInterval = 500;   // store phase space after time intervals
   ...
   WriteSpatialFieldsProfile = false;
 ...
 void LEADS_Input :: ParameterInfo()
 {
   ...
   PulseModel:
   		...
   		2 - Subcycle Pulsed Focused Vector Beams
 	       ...
 	       4 - Plane wave with exponential decay. Vector potential dependent
 #endif
